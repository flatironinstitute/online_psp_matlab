...
hold all
legends={};
cm=colormap(hot(numel(files)+15));
d_s=[];
q_s=[];
err_s=[];
errhalf_s=[];
for f=1:numel(files)
   disp(files(f).name)
   load(files(f).name,'d','q','errors_real','errors_decorr')
   legends{f}=files(f).name;
   
   subplot(2,1,1)
   hold all
   plot((median(errors_real,2)),'d','Linewidth',2)
   subplot(2,1,2)
   hold all
   plot((median(errors_decorr,2)),'d','Linewidth',2)
