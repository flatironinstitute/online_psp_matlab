...
options_simulations.initialize_PCA=1;
...
options_simulations.error_online=0;
options_algorithm=struct;
options_simulations.orthonormalize_vectors=0;
...
t = datetime('now');
folder_exp=['DEC_n0_' num2str(options_simulations.n0) '_niter' num2str(options_simulations.niter) '_' char(t)];
mkdir(folder_exp)
hold all
legends={};
options_generator.n=4096;
cm=hot(220);
counter=0;

for q=16%[2 4 16 64 256 512]
    for d=128%[4 16 64 256 1024]
        for rho=.5%.1:.2:.9
            for lambda=0:.2:1
...
%                 end
                ...
                options_algorithm.lambda=lambda;
                [errors_real,errors_batch_pca,errors_online,times_,fname]=Online_PCA_simulations(folder_exp,options_simulations,options_generator,options_algorithm);
                if ~isempty(errors_real)
                    axs=[];
                    counter=counter+1;
